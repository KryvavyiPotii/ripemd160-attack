...
fn main() {
    // Initialize logging of all levels to stdout.
    Builder::new()
        .format(|buf, record| 
            writeln!(
                buf,
    ...
    let matches = Command::new("ripemd160-attack")
        ...
        .arg(
            Arg::new("attack count")
                .short('c')
                .long("attack-count")
                .default_value(DEFAULT_ATTACK_COUNT)
                .value_parser(clap::value_parser!(u64))
                .help("Number of attacks to execute")
        )
        .arg(
            Arg::new("transform message")
                .long("tr")
                .action(ArgAction::SetTrue)
                .requires("attack count")
                .help("Transform initial message between attacks")
        )
        .arg(
            Arg::new("message")
                .short('m')
        ...
        .arg(
            Arg::new("message transform type")
                .long("tr-type")
    ...
    let attack_count = matches
        .get_one::<u64>("attack count")
        .unwrap();
    let transform_message = if *attack_count > 1 {
        matches
            .get_flag("transform message")
    }
    else {
        false
    };
    ...
    let message_transform = match matches
        .get_one::<String>("message transform type")
        .unwrap()
        .as_str()
    {
        TRANSFORM_RANDOM_NUMBER => 
        ...
        _ => {
            error!("Invalid message transform");
            process::exit(-1);
        }
    };
